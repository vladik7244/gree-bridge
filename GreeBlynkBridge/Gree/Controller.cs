namespace GreeBlynkBridge.Gree
{
    ...
    public class Controller
    {
        ...
        public async Task SetDeviceParameters(List<Param> parameters)
        {
            this.log.LogDebug($"Setting parameters: {parameters}");

            var pack = CommandRequestPack.Create(
                this.DeviceID,
                parameters.Select(a => a.name).ToList(),
                parameters.Select(a => a.value).ToList()
            );
            ...
            {
                if (!responsePack.Columns.Contains(parameter.name))
                {
                    this.log.LogWarning($"Parameter {parameter.name} cannot be changed.");
                }
            }
        }
